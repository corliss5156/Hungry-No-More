...
def item(update,context):
    ...
    for shop in (shopList):
        for menu in shop['items']:
            button_list.append(telegram.InlineKeyboardButton(menu,callback_data=menu))
    reply_markup=telegram.InlineKeyboardMarkup(build_menu(button_list,n_cols=1))
...
def order_callback(update,context): 
    ...
    item = update.callback_query.message.text
    ...
    itemList = database['items'].find({'itemName': menu_item})
    credits = itemList[0]['credits']
