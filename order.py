...
def record(update, context): 
    if update.message.text == "Cancel": 
        ...
        if context.chat_data['user_credits'] < int(credits): 
            ...
            try: 
                record_transaction_buyer(user, int(credits), store, order_item, date)
...
def record_transaction_buyer(username, credits, store, menu_item, date): 
    database.buyer_transactions.insert({'user': username, 'transaction': - credits,'store': store, 'menu_item': menu_item, 'date': date})
    
def record_transaction_seller(username, credits, menu_item, date): 
    database.seller_transactions.insert({'user': username, 'transactions': + credits, 'menu_item': menu_item, 'date': date})
